...
const Field = React.forwardRef(({ label, error, required = false, type = 'text', placeholder = '', ...rest }, ref) => (
  <div className="flex flex-col gap-1">
    ...
    <input
      ...
      className={`px-3 py-2 text-sm focus:outline-none bg-white transition ${
    ...
    <form onSubmit={onSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      {isEigenaar && (
        <div className="flex flex-col gap-1">
          ...
          <select className="border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-black bg-white" {...register('role')}>
      ...
      {serverError && <p className="col-span-2 text-red-600 text-xs font-medium">{serverError}</p>}
